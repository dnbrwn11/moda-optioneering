...
const SEASON = getAppConstants().season_windows
export const OFFSEASON_MONTHS = SEASON.offseasonMonths
export const DURING_SEASON_MONTHS = SEASON.duringSeasonMonths
const MONTHS_PER_YEAR = 12

// Season window month boundaries (calendar month, 1 = Jan … 12 = Dec), served
// from app_constants so downstream views (the analytics cash-flow S-curve)
// get month-accurate band edges from the same source as the CONT split.
export const SEASON_WINDOWS: {
  offseason: { startMonth: number; endMonth: number }
  duringSeason: { startMonth: number; endMonth: number }
} = {
  offseason: SEASON.offseason,
  duringSeason: SEASON.duringSeason,
}
