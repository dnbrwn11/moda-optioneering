...
import { tradeWindowSpend } from '../lib/resources'
import {
  DELIVERY_CONDITIONS,
  DELIVERY_NOTE,
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] border-collapse text-xs">
            ...
            <tbody>
              {(
                [
                  ['Craft', (w: (typeof workforce.windows)[number]) => w.craftHours],
                  ['Apprentice', (w: (typeof workforce.windows)[number]) => w.apprenticeHours],
                  ['Local hire', (w: (typeof workforce.windows)[number]) => w.localHours],
                ] as const
              ).map(([label, pick]) => (
                <tr key={label} className="border-b border-line/60">
                  ...
                  {workforce.windows.map((w) => {
                    ...
                    return (
                      ...
                      >
                        {v >= 0.5 ? fmtHours(v) : <span className="text-line">·</span>}
                      </td>
                    )
                  })}
                </tr>
