...
import { getAppConstants, getCatalogData, getSession } from './boot/catalog'
...
import {
  ...
  BASELINE_TOTALS,
  MAX_USER_SCENARIOS,
  resolveInitialPersisted,
  sanitizeRemotePersisted,
  SEEDED_ITEMS,
  snapshotFrom,
} from './lib/scenarios'
import type { PersistedState, Scenario } from './lib/scenarios'
import { initPersistence, schedulePersist } from './lib/persistence'
...
function logStartupAudit(items: Item[], baselineEscalated: number): void {
  ...
  const band = getAppConstants().startup_audit_band
  const target = baselineEscalated >= band.min && baselineEscalated <= band.max
  console.log(
    `%cValidation: escalated total ${target ? 'WITHIN' : 'OUTSIDE'} $${(band.min / 1e6).toFixed(0)}-${(band.max / 1e6).toFixed(1)}M target`,
...
// Boot from the user's remote rows when they exist — restoring saved
// scenarios AND the exact working state (unsaved edits survive a reload),
// optionally merged with a one-time accepted localStorage import.
// applySnapshot reconciles the persisted working state against the current
// catalog, so a catalog change can never break boot.
const bootSession = getSession()
const remotePersisted = sanitizeRemotePersisted(bootSession.rawPersisted)
const persisted = resolveInitialPersisted(
  remotePersisted,
  bootSession.pendingImport,
  bootSession.workingRowExists,
)
...
// no mutation path can be missed (debounce + dirty-diff live in
// lib/persistence.ts). initPersistence arms the writer with what is actually
// in the database, so nothing can write until the hydrated state is in place.
const payloadFrom = (state: AppState): PersistedState => ({
  version: 1,
...
initPersistence(remotePersisted)
useStore.subscribe((state) => schedulePersist(payloadFrom(state)))
// One unconditional kick: a first login creates the working row, an accepted
// localStorage import lands immediately, and an unchanged boot diffs to
// nothing and writes nothing.
schedulePersist(payloadFrom(useStore.getState()))
